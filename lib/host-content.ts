...
export const hostContentMap: Record<string, SpokeContent> = {
  ...
  portfolio: {
    ...
    body: [
      "A portfolio only earns you anything if the person you sent it to actually opens it. Behance and Dribbble wrap your work in their own navigation, ads, and a 'suggested creators' rail that pulls attention toward other people. A Google Drive folder makes a recruiter sign in before they see a single piece, while a 40MB PDF attachment gets stripped by a corporate mail server or sits unopened because nobody downloads files from someone they have not met. A plain link sidesteps all of it. Your work opens directly, on any device, with nothing in front of it.",
  ...
  xlsx: {
    ...
    body: [
      ...
      "Hosting matters in a few specific cases: sending a model to a client who shouldn't be able to edit it by accident, sharing data with a journalist who's on a phone, or needing a permanent URL that doesn't disappear from a Slack DM after the workspace retention window. Each of these is a NudgeHost-shaped problem, and the same uploader that lets you {{host-hub}} handles the PDFs that travel alongside the XLSX, and you can {{host-docx}} from the same dashboard.",
