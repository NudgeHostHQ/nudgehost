...
export const hostContentMap: Record<string, SpokeContent> = {
  ...
  html: {
    ...
    body: [
      ...
      "NudgeHost takes any HTML file, whether a single page or a multi-file site, and puts it on the open web at a clean URL within seconds. If your project is more than one file, the simplest route is to zip everything together and {{host-zip}}; NudgeHost unpacks it and serves it as a site. If you're publishing something a model built for you, there's a purpose-built flow to {{host-claude-artifact}} that handles the copy-paste case directly.",
