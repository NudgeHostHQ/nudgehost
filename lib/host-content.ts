...
export const hostContentMap: Record<string, SpokeContent> = {
  ...
  html: {
    ...
    body: [
      ...
      "NudgeHost takes any HTML file, whether a single page or a multi-file site, and puts it on the open web at a clean URL within seconds. If your project is more than one file, the simplest route is to zip everything together and {{host-zip}}; NudgeHost unpacks it and serves it as a site. If you're publishing something a model built for you, there's a purpose-built flow to {{host-claude-artifact}} that handles the copy-paste case directly. For personal use cases like a one-page event site, the guide on how to {{use-case-wedding}} walks through that flow start to finish.",
  ...
  "lovable-export": {
    ...
    body: [
      ...
      "The export from Lovable is a ZIP of the built React app. Drop the ZIP here and NudgeHost unpacks it, serves the index.html, and rewrites unknown paths to that index so React Router works on direct URL visits. It's the same pipeline you'd use to {{host-react-app}} directly; the only difference is that the source happened to be an AI builder rather than a hand-built Vite project. The result is identical: a clean URL anyone can open.",
  ...
  "v0-export": {
    ...
    body: [
      ...
      "All of this works on {{pricing}}. Pro adds custom domains and password protection, both worth it when the URL goes to a client. The dashboard handles {{host-hub}} alongside the v0 demo. Project bundles work directly too: just {{host-zip}} and the archive sits at its own link.",
    ],
    faqs: [
      {
  ...
  gif: {
    ...
    body: [
      ...
      "There's a size trap worth knowing about. A two-second animated GIF can easily be 8MB. The same animation as an MP4 with H.264 encoding is usually under 200KB and looks identical. If your GIF is more than five seconds long, converting to MP4 first is almost always the right call: faster page loads, less bandwidth, no visible quality drop. You can {{host-mp4}} from the same dashboard once it's encoded. GIF hosting is the right pick when you specifically need GIF (forum software that won't accept MP4, Slack reactions, certain Reddit subs).",
