...
export const glossaryContentMap: Record<string, GlossaryContent> = {
  ...
  cdn: {
    ...
    body: [
      ...
      "On the free plan your files already ride this network, and {{pricing}} keeps that true at higher limits when you move to a paid tier. You never configure any of it. There is no region to pick and no cache to warm by hand. The point of a CDN for someone sharing a file is simply that they never have to think about it.",
    ],
    faqs: [
      {
  ...
  https: {
    ...
    body: [
      ...
      "Every NudgeHost link is HTTPS by default, with no setting to toggle and nothing to install. This matters more than it sounds, because modern browsers actively label plain HTTP pages as not secure, and a recruiter or client who sees that warning on a link you sent will hesitate before clicking. Serving a shared document over a secure connection removes that friction. The encryption is handled by {{glossary-ssl-certificate}}, which the host provisions and renews for you.",
      "Security is also why you can share genuinely sensitive material as a link. A contract you {{host-pdf}} travels encrypted, and you can layer password protection on top when the contents warrant it. Hosting is free to begin with, and {{pricing}} adds custom domains that keep the same automatic HTTPS on your own address. The takeaway is that secure delivery is the default here, not an upgrade.",
    ],
    faqs: [
      {
  ...
  cache: {
    ...
    body: [
      ...
      "For shared files this is why the second open is quicker than the first. The first visitor pulls the file to a nearby edge server, and everyone after them is served that cached copy at once. NudgeHost leans on this through the network that powers how a CDN works, so a page you {{host-html}} stays fast even under a burst of traffic. Heavy assets benefit most, which is why an optimized image you {{converter-png-to-webp}} before uploading caches small and serves quickly.",
      "Caching has one wrinkle worth knowing. When you replace a file, a previously cached copy can linger briefly until it refreshes, so a change is not always visible to everyone in the same instant. This usually clears in moments. Hosting is free to start and {{pricing}} covers the higher limits. For the person sharing a file, the cache is invisible and entirely upside, meaning faster loads with nothing to manage.",
    ],
    faqs: [
      {
  ...
  "qr-code": {
    ...
    body: [
      ...
      "For shared files this matters more than it first appears. A printed wedding invitation can carry a code that opens the details page you set up when you {{use-case-wedding}}, and a poster can point to a PDF you {{host-pdf}} without anyone squinting at a long URL. NudgeHost generates a QR code for every link automatically, and it does so on the free plan, where competitors typically gate this behind a paid tier. There is nothing to generate separately and no third-party tool to trust with your URL.",
      "Because the code simply points at your link, it inherits everything the link does. Update the file and the code keeps working, since the URL has not changed. Set the link to expire and the code expires with it. Hosting and the QR code are free, and {{pricing}} only enters the picture for things like custom domains. A QR code is the most direct way to hand someone a file without handing them a keyboard.",
    ],
    faqs: [
      {
  ...
  "drag-and-drop": {
    ...
    body: [
      ...
      "NudgeHost is built around it. You {{home}} by dropping a file onto the uploader, and a clean link comes back in seconds, which is the whole point of the product compared to the old {{glossary-ftp}} workflow it stands in for. There is no client to install and nothing to configure. On a phone the same control falls back to tap-to-browse, so the pattern works whether you are at a desk or sharing something from the train.",
  ...
  "password-protection": {
    ...
    body: [
      ...
      "On NudgeHost the password is stored only as a secure hash, never as plain text, and entering the right one quietly opens the file for that visitor. Hosting is free to start, and {{pricing}} carries the access controls across your files. The honest framing is that password protection is not heavy security for state secrets; it is the practical lock that stops an ordinary shared link from being opened by the wrong person.",
    ],
    faqs: [
      {
