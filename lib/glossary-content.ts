...
export const glossaryContentMap: Record<string, GlossaryContent> = {
  ...
  ftp: {
    ...
    body: [
      ...
      "The practical difference is time. FTP is minutes of setup per server; dropping a file is seconds, and the result is a clean {{features-shareable-links}} rather than a path buried on a server you now have to maintain. Hosting is free to start, and the limits only matter when a file is genuinely large. FTP still has a place in old-school server administration, but for getting a file in front of someone it is the long way round.",
    ],
    faqs: [
      {
  ...
  "drag-and-drop": {
    ...
    body: [
      ...
      "The reason this matters beyond convenience is that it removes the failure points. There is no wrong directory to drop a file into and no credential to fumble, so the path from file to link is short and hard to get wrong. When what you are publishing is markup rather than a file, {{features-paste-html|paste mode}} covers the no-file case. Drag and drop is not a flourish here; it is the entire upload model.",
    ],
    faqs: [
      {
