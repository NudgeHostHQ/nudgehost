...
export const useCasesContentMap: Record<string, SpokeContent> = {
  ...
  "send-large-pdf-without-email": {
    ...
    body: [
      ...
      "This works for any large file, not just PDFs. The same dashboard handles {{host-hub}}, and a collection of files zips up so you can {{host-zip}} as one link. If the document is sensitive, link expiry and password protection turn the share into something you control after sending, and the file is served through a {{glossary-presigned-url}} so the storage behind it stays private.",
