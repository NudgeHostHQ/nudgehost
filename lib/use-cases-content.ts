...
export const useCasesContentMap: Record<string, SpokeContent> = {
    ...
    faqs: [
      {
        ...
        a: "If it's a site, zip the folder and it serves as a browsable site at one link, navigation intact. For a loose set of documents, host each file and lead with the strongest piece; a recruiter rarely opens more than two links.",
      },
      {
  ...
  "send-large-pdf-without-email": {
    ...
    body: [
      ...
      "This works for any large file, not just PDFs. The same dashboard handles {{host-hub}}, with each file at its own link. If the document is sensitive, link expiry and password protection turn the share into something you control after sending, and the file is served through a {{glossary-presigned-url}} so the storage behind it stays private.",
  ...
  "share-deck-with-client": {
    ...
    body: [
      ...
      "The open-tracking is quietly valuable in client work. You'll know whether the deck was reviewed before a meeting, which shapes how you run the call. For sensitive commercial decks, you can {{features-password-protection}} or set an expiry. If you're sending supporting documents alongside the deck, {{host-hub}} can take each file one at a time, so the brief, the agenda, and the data each get their own link.",
  ...
  "share-wedding-website": {
    ...
    body: [
      ...
      "NudgeHost is the much smaller, much cheaper middle ground. Build your wedding page however you want (a single HTML file you wrote, an AI-generated layout, a PDF designed in Canva, a Notion export printed to PDF) and you can {{host-html}}; if you went the PDF route, the {{host-pdf}} handles that the same way. The result is a clean nudgehost.com link with no banner ads, no upsells, and no $30 annual renewal. If you'd rather share a designed image of the invitation, you can {{host-image}} the same way. A page that grew into a small site, with a photo page and a directions page, zips up so you can {{host-zip}}; the whole site lives at one URL. When the venue confirms a final start time or you spot a typo, swap the source file in your dashboard and the URL stays the same, so the version every guest sees updates instantly. Print the same link as a {{glossary-qr-code}} on the invitation and older relatives can open it with a phone camera instead of typing a URL.",
